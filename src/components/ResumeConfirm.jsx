    ...
    <div className="resume-confirm-overlay" onMouseDown={(e) => { if (e.target === e.currentTarget) onCancel(); }}>
      <div className="resume-confirm">
        <div className="resume-confirm-head">
          <h3>这是我读出来的，你改改</h3>
          <button type="button" aria-label="关闭简历确认" className="resume-confirm-close" onClick={onCancel}>✕</button>
        </div>
        <p className="muted">这份简历我读出了下面这些，你可以直接改；确认后才放进你的处境卡。姓名和联系方式不会保留。</p>
        <div className="resume-confirm-body">
          ...
          <label>项目经历（每行格式：项目名|角色|亮点1，亮点2）
            <textarea aria-label="项目经历" rows={3} value={form.projects} onChange={(e) => update('projects', e.target.value)} placeholder="智能推荐系统|后端负责人|QPS 提升 30%，成本下降 20%" />
          </label>
          <div className="resume-confirm-row">
            <label>技能（用顿号分隔）
        ...
        <div className="resume-confirm-actions">
          ...
          <button className="chip primary" onClick={confirm}>就用这些</button>
        </div>
      </div>
    </div>
  );
