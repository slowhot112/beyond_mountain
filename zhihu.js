...
严格要求：只返回一个 JSON 对象，不要任何额外文字、不要 markdown 代码块。所有用户可见字段使用「山径、路标、脚印、判断、行动簿」这一套语言，不得使用旧产品术语「炼金、炼金包、炼过」。
  ...
  if (bucket === 'urgent') {
    ...
    core = [
      mk('第1周 · 岗位与行业认知',
        ...
        `①学（周一）：读 2 份这个岗位的行业资料/白皮书，写 1 页概念卡；②拆（周二）：拆 3 条真实 JD，填"要求/工具/场景/门槛"四列表；③练（周三）：再收 7 条凑满 10 条，统计词频，建立可比较的基线；④做（周四）：扩到 ≥30 条去重 JD 并完成四列分类，写出《岗位地图》；⑤补（周五）：专门挑 5 条门槛异常高或要求互相矛盾的 JD 作为反例，标出"哪些是我现在进不去的"；⑥验（周末）：对照 JD 写出《个人能力差距清单》，找 1 位从业者看一遍并请他指出"你觉得我差得最多的是哪项"。`,
      ...
      mk('第4~5周 · 做出可演示的作品',
        ...
        `①学（周一）：确定作品范围，写 1 页 一页作品说明（一句话问题/目标用户/核心任务/输入/输出/成功指标/明确不做的）；②拆（周二）：画出用户流程与数据流程，列出正常、空白、加载、失败、超时等状态；③练（周三）：先用模拟数据把核心流程跑通，建立可比较的基线；④做（周四~周五）：接上真实能力（模型/数据/外部服务），做出能用的版本，记录优化前/后；⑤补（第2周前半）：补边界与反例（空输入、超长输入、它答不了的情况），修掉影响核心流程的问题；⑥验（第2周周末）：部署成可公开访问的链接，找 3 名没参与的人试用，记录完成率、用时、卡点和原话。`,
    ...
    core = [
      mk('第1~2周 · 岗位与行业认知',
        ...
        `①学（第1周前半）：读 2 份行业资料或白皮书，写 1 页概念卡，画出这个岗位所在的产业链；②拆（第1周中段）：拆 5 条真实 JD，填"要求/工具/场景/门槛"四列表；③练（第1周后半）：收集到 20 条 JD，统计词频，建立可比较的基线；④做（第2周前半）：扩到 ≥50 条去重 JD（有余力到 100 条）并完成四列分类；⑤补（第2周中段）：挑 10 条门槛异常高或要求互相矛盾的 JD 作为反例，标出"哪些我现在进不去"；⑥验（第2周周末）：写《个人能力差距清单》，找 1 位从业者看一遍，请他指出"你觉得我差得最多的是哪项"。`,
      ...
      mk('第9~10周 · 做出可演示的作品',
        ...
        `①学（第9周前半）：写 1 页 一页作品说明（一句话问题/目标用户/核心任务/输入/输出/成功指标/明确不做的范围）；②拆（第9周中段）：画用户流程与数据流程，列出正常、空白、加载、失败、超时等状态；③练（第9周后半）：先用模拟数据把核心流程跑通，建立可比较的基线；④做（第10周前半）：接上真实能力（模型/数据/外部服务），做出能用的版本，记录优化前/后；⑤补（第10周中段）：补边界与反例（空输入、超长输入、它答不了的情况），把前面两周的测试样本接进来，修掉影响核心流程的问题；⑥验（第10周周末）：部署成可公开访问的链接，找 3 名没参与的人试用，记录完成率、用时、卡点和原话。`,
...
function summarizeHistory(rec) {
  ...
  parts.push(`「${t}」`);
  if (dom) parts.push(`本轮答题更多接近「${dom}」`);
...
export function buildHistoryBlock(selected) {
  ...
  return `
你过去走过的、与本次问题相关的山径记录（★仅供参考；若与本次问题无关，请完全忽略，绝不要被它带偏结论，也不要套用它的行业默认设定）：
${lines}
...
第4周 做出可运行作品：把选题做成能跑通的版本（输入→处理→产出→结果可核验）→ 记录优化前/后；
...
每周的 steps 要能看出一条推进线：先拿输入（补一点必要的认知、拆一个真实的 JD/案例）→ 建立可比较的基线 → 做出本周的交付物 → 用 刁钻情况、异常输入、反例去砸它 → 交给真人验收并复盘。
...
6. role 分配：先给本轮答题最接近的观点 1~2 条（验证它是否站得住，role=${dominantId || 'r1'}）；盲区视角 ${uncertain.length ? uncertain.map((u) => roleName(u) + '(' + u + ')').join('、') : '无'} 每个至少 1 条；其余覆盖别的观点。
